...
const PLATFORMS = [
  {
    ...
    Logo: () => (
      <svg viewBox="0 0 24 24" fill="currentColor" className="h-8 w-8 text-[var(--lp-text-soft)]">
  ...
  },
  {
    ...
    Logo: () => (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="h-8 w-8 text-[var(--lp-text-soft)]">
...
export function PlatformsSection() {
  return (
    <section className="bg-[var(--lp-bg)] px-5 sm:px-8 md:px-10 py-24 sm:py-32 transition-colors duration-300">
      <div className="mx-auto max-w-6xl">

        {/* Header */}
        <div className="mb-16 sm:mb-20 flex flex-col md:flex-row md:items-end md:justify-between gap-8">
          <FadeIn>
            <p className="text-[10px] uppercase tracking-[0.3em] text-[var(--lp-text-muted)] mb-5">
              Powered by
            </p>
            <h2
            ...
            >
              <span className="block text-[var(--lp-gold)]">6 Platforms.</span>
              <span className="block text-[var(--lp-text)]">One Chart.</span>
            </h2>
          </FadeIn>

          <FadeIn delay={0.15}>
            <p className="text-[var(--lp-text-muted)] leading-relaxed max-w-sm text-sm sm:text-base md:text-right">
          ...
          {PLATFORMS.map((platform, i) => {
            ...
            return (
              <FadeIn key={platform.name} delay={i * 0.07}>
                <div className="group border border-[var(--lp-card-border)] rounded-2xl sm:rounded-3xl p-5 sm:p-6 bg-[var(--lp-card-bg)] flex flex-col gap-5 hover:border-[var(--lp-border-strong)] hover:shadow-sm transition-all duration-300">
                  <div className="flex items-start justify-between">
                    <Logo />
                    <span className="flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                      <span className="text-[9px] uppercase tracking-widest text-green-600/80 hidden sm:block">
                        Live
                      </span>
                    </span>
                  </div>
                  <div>
                    <p className="text-[var(--lp-text)] font-semibold text-sm sm:text-base">
                      {platform.name}
                    </p>
                    <p className="text-[10px] sm:text-xs text-[var(--lp-text-muted)] mt-1 uppercase tracking-widest">
                      {platform.type}
                    </p>
                  </div>
                </div>
        ...
        {/* Stats bar */}
        <FadeIn delay={0.5}>
          <div className="mt-12 sm:mt-16 border-t border-[var(--lp-border)] pt-10 grid grid-cols-2 sm:grid-cols-4 gap-8">
            {STATS.map((stat) => (
              <div key={stat.label}>
                <p className="text-3xl sm:text-4xl font-black text-[var(--lp-text)] tracking-tight">
                  {stat.value}
                </p>
                <p className="text-[10px] uppercase tracking-[0.25em] text-[var(--lp-text-muted)] mt-2">
                  {stat.label}
                </p>
              </div>
            ))}
