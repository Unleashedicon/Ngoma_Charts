...
const F = "'Kanit','Instrument Sans',Helvetica,sans-serif";
const SF = "'Kanit','Source Serif 4',Georgia,serif";
...
export default function NgomaCharts(){
  const [page,setPage]=useState(()=>{
    ...
    return VALID_PAGES.includes(hash) ? hash : "charts";
  });
  ...
  useEffect(()=>{
    const onHash=()=>{const h=window.location.hash.slice(1);if(VALID_PAGES.includes(h))setPage(h);};
...
const top = data[0];
  ...
  const navTo=p=>{setPage(p);window.location.hash=p;setSelA(null);setSelR(null);setSelNews(null);setMNav(false);setMoreOpen(false);};
